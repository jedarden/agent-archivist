...
Example body (field names illustrative until the Phase 1 schema):
...
```json
{
  ...
  "request_id": "6bc6d1e0f2a54c9b8e3d7a1f0c5b2e4d",
  "correlation_id": "9f2a51b0e8c74d6a3b0f1e2d3c4b5a69"
}
```
...
- **ERR-025** — Every logical upload request carries a `request_id`: 32
  lowercase hex characters (128 random bits) generated by the client when
  the request is frozen, reused across every retry of that request, and
  bound into the envelope, attestation, receipt, and error bodies. It is a
  correlation handle, not a security identity (ID-002's caution applies).
